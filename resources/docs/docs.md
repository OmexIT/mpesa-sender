...
The `home-routes` handler in the `mpesa-sender.routes.home` namespace
...
The `home-page` function will in turn call the `mpesa-sender.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `mpesa-sender.handler` namespace:
...
Request middleware functions are located under the `mpesa-sender.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `mpesa-sender.dev-middleware` namespace found in
...
* Let `mount` know to start the database connection by `require`-ing `mpesa-sender.db.core` in some other namespace.
